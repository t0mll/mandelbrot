...
/// where `<sep>` is the character given by the `separator` argument,
...
/// If it doesn't parse corrently, return `None`.
...
fn pixel_to_point(
    ...
    Complex {
        ...
        im: upper_left.im - pixel.1 as f64 * height / bounds.1 as f64, // Why substaction here? pixel.1 increases as we go down, but the imaginary component
...
/// grayscale pixel per byte.
/// The `upper_left` and `lower_right` arguments specify points on the complex plane corresponding
